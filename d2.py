...
def is_any_attack_running():
    with _attack_lock:
        return len(active_attacks) > 0

def get_running_attacks_info():
    with _attack_lock:
        info = []
        for attack_id, attack in active_attacks.items():
            remaining = (attack['end_time'] - datetime.now()).total_seconds()
            if remaining > 0:
                info.append({
                    "target": attack.get('target', 'Unknown'),
                    "port": attack.get('port', 'Unknown'),
                    "user_id": attack.get('user_id', 'Unknown'),
...
def handle_attack(message):
    ...
    if is_any_attack_running():
        running_attacks = get_running_attacks_info()
        attack_msg = "❌ **Abhi ek attack already chal raha hai!**\n\n"
        attack_msg += "📊 **Currently Running Attacks:**\n"
        for atk in running_attacks:
            attack_msg += f"   🎯 `{atk['target']}:{atk['port']}` | ⏱️ {atk['remaining']}s remaining | 👤 User: `{atk['user_id']}`\n"
        attack_msg += "\n⏳ Please wait for the current attack to finish before starting a new one!\n"
        attack_msg += f"🕐 Approximate wait time: {running_attacks[0]['remaining'] if running_attacks else 0} seconds"
        bot.reply_to(message, attack_msg, parse_mode="Markdown")
        return
...
print("="*60)
print("🎯 ATTACK CONFLICT DETECTION: ENABLED")
print("   • Agar koi attack chal raha hai to naya attack start nahi hoga")
print("   • User ko wait karne ko kaha jayega")
print("="*60)
